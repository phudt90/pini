...
(function( clevermenu, $ ) {
    ...
    $.extend( clevermenu, {
        options : {
            ...
            break_point: 992,
            selector : {
                megaMenuSelector                : ".mmm",
                subMegaMenuContainerSelector    : ".mmm-content-container",
                subMegaMenuWrapperSelector      : ".mmm-content-wrapper"
            }
        },
        MegaMenu : {
            init: function() {
                $( clevermenu.options.selector.megaMenuSelector ).each( function() {
                    var options_data = {"menuStyle":"horizontal", "parentSelector":".mmm", "breakPoint":"992"};
            ...
            build: function( menu ) {
                ...
                $( 'li.mmm-mega', menu ).each( function () {
                    var item = $( this ),
            ...
            mobile: function( menu ){
                ...
                var dropdownToggle = $( '<button />', { 'class': 'mmm-dropdown-toggle', 'aria-pressed': false }).append( '<i class="dashicons dashicons-arrow-down"></i>' );
                ...
                menu.find( '.mmm-dropdown-toggle' ).on( 'click', function( e ) {
                    var _this = $( this );

                    e.preventDefault();

                    _this.toggleClass( 'clever-toggled-on' );
                    _this.next( '.children, .sub-menu, .mmm-sub-container, .mmm-content-container' ).toggleClass( 'clever-toggled-on' );
...
(function( $ ) {
    ...
    var cleverMenuContainer = $( '.mmm-container' );

    cleverMenuContainer.each( function() {
        var cleverMegaMenu = $( this ).find( '.mmm' );
        var cleverMenuContainerId = $( this ).attr( 'id' );
        var cleverMenuContainerSelector = $('#' + cleverMenuContainerId);
        var mobileConfig = {"toggleDisable":"0","toggleWrapper":".mmm-container","ariaControls":"","toggleIconOpen":"dashicons dashicons-menu","toggleIconClose":"dashicons dashicons-no-alt","toggleMenuText":"Menu"};
        ...
        if ( mobileConfig.toggleDisable === '0') {;
            //cleverMobileMenuSelector.prepend( '<div id="' + cleverMenuContainerId + '-toggle" class="' + cleverMenuContainerId + '-toggle mmm-toggle-wrapper"><button class="mmm-toggle" aria-controls="' + mobileConfig.ariaControls + '" aria-pressed="false"><span class="toggle-icon-open ' + mobileConfig.toggleIconOpen + '"></span><span class="toggle-icon-close ' + mobileConfig.toggleIconClose + '"></span><span class="toggle-text">' + mobileConfig.toggleMenuText + '</span></button></div>' );
        }

        // var cleverMenuToggle = $(this).find( '.mmm-toggle' );
        var cleverMenuToggle = cleverMobileMenuSelector.find( '#' + cleverMenuContainerId + '-toggle' ).children( '.mmm-toggle' );
        ...
        (function() {
            ...
            function cleverToggleFocusClassTouchScreen() {
                if ( 'none' === $( '.mmm-toggle' ).css( 'display' ) ) {

                    $( document.body ).on( 'touchstart', function( e ) {
                        if ( ! $( e.target ).closest( '.mmm-container li' ).length ) {
                            $( '.mmm-container li' ).removeClass( 'focus' );
                        }
                    });
